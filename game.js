...
function autoSelectShots() {
  ...
  let validAttackers = attackers.filter(atk => attackerPositions[atk.id] && attackerPositions[atk.id].length > 0);
  if (validAttackers.length === 0) {
    ...
    if (emptyCells.length > 0) {
      ...
      actions.push("Auto-selected random shot at " + String.fromCharCode(65 + randomCell[1]) + (randomCell[0] + 1));
    }
    return;
  }
  ...
  for (let atk of validAttackers) {
    ...
    do {
      ...
      if (isValid) {
        ...
        actions.push("Auto-selected shot targeting attacker " + atk.id + " at " + String.fromCharCode(65 + predictedPos[1]) + (predictedPos[0] + 1));
        return;
      }
      attempts++;
...
function nextTurn() {
  ...
  for (let atk of attackers) {
    if (atk.currentIndex < atk.steppedPath.length - 1) {
      ...
      if (shotHit) { actions.push("Attacker " + atk.id + " was hit at " + String.fromCharCode(65 + nextTile[1]) + (nextTile[0]+1)); continue; }
      else {
        ...
        if (atk.currentIndex === atk.steppedPath.length - 1 || (atk.speed===2 && atk.currentIndex===atk.steppedPath.length-2)) {
          ...
          actions.push("Attacker " + atk.id + " reached and destroyed defender at " + String.fromCharCode(65 + atk.baseTarget[1]) + (atk.baseTarget[0]+1) + " and was destroyed");
      ...
      board[atk.baseTarget[0]][atk.baseTarget[1]] = 0;
      actions.push("Attacker " + atk.id + " destroyed defender at " + String.fromCharCode(65 + atk.baseTarget[1]) + (atk.baseTarget[0]+1) + " and was destroyed");
      redirectAttackers(atk.baseTarget);
    }
  }
